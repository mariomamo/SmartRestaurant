...
import org.junit.runner.RunWith;import it.unisa.smartrestaurantapp.R;
...
public class AggiungereUnTavoloTC2101 {
    @Rule
    ...
    public void useAppContext() throws InterruptedException {
        ...
        //Inserisco i dati
        Espresso.onView(withId(R.id.username)).perform(typeText("T@volo150"));
        Espresso.onView(withId(R.id.password)).perform(typeText("pass"));
        Espresso.onView(withId(R.id.nome)).perform(typeText("Crystal"));
        Espresso.onView(withId(R.id.posti)).perform(typeText("6"));

        //Clicco il tasto "Slva" e aspetto che le modifiche vengano eseguite
        Espresso.onView(isRoot()).perform(ViewActions.pressBack());
        Espresso.onView(withId(R.id.salva)).perform(click());
        Espresso.onView(withText("Username non rispetta il formato")).inRoot(new ToastMatcher()).check(matches(isDisplayed()));
    }
}